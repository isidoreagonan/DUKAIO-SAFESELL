...
import { ArrowLeft, Eye, Globe, History, Layers, Loader2, MoreHorizontal, RotateCcw, Save, Sparkles, Upload, X } from "lucide-react";
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
function EditeurPage() {
  ...
  return (
    <div className="flex h-[calc(100vh-4rem)] min-h-0 flex-col">
      <header className="flex flex-col gap-3 border-b border-border bg-card px-4 py-3 sm:flex-row sm:flex-wrap sm:items-center md:grid md:grid-cols-[1fr_auto_1fr]">
        ...
        <div className="flex items-center gap-2 md:justify-self-end">
          ...
          <button
            ...
            disabled={busy}
            className="btn-3d flex flex-1 items-center justify-center gap-1.5 px-3 py-2 text-sm font-semibold disabled:opacity-60"
          >
            {pending === "save" ? (
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <button
                ...
                className="flex size-9 shrink-0 items-center justify-center rounded-[6px] border border-border hover:bg-accent disabled:opacity-60"
              >
              ...
              {online ? (
                <>
                  ...
                  <DropdownMenuItem onSelect={() => void persist("unpublish")}>
                    <Globe size={14} /> Dépublier la boutique
                  </DropdownMenuItem>
                </>
              ) : null}
            </DropdownMenuContent>
      ...
      <div className="flex min-h-0 flex-1 flex-col overflow-hidden md:flex-row">
        ...
        <div className={cn("min-h-0 flex-1 md:contents", mobileView !== "sections" && "hidden")}>
          <EditorSidebar onReset={reset} />
        </div>
        <div className={cn("flex min-h-0 flex-1 flex-col md:contents", mobileView !== "apercu" && "hidden")}>
          <LivePreview />
        </div>
