...
import { ArrowRight, BarChart3, Bot, Check, Compass, Megaphone, PackageCheck, Palette, Rocket, ShoppingBag, Sparkles, Store, Warehouse } from "lucide-react";
  ...
  return (
    <div className="divide-y-2 divide-foreground/10 border-y-2 border-foreground/10">
      {benefits.map(({ icon: Icon, title, text }, i) => (
        <article
          key={title}
          className="grid gap-4 py-7 md:grid-cols-[5rem_1fr_1fr] md:items-center"
        >
...
export const plans = [
  {
    name: "Découverte",
    price: "0",
    note: "14 jours pour lancer votre boutique et tester l'IA DUKAIO.",
    recommended: false,
    icon: Store,
    cta: "Commencer gratuitement",
    items: [
      "1 boutique en ligne publiable et lien partageable",
      "Jusqu'à 20 produits dans votre catalogue",
      "1 création de page produit par IA offerte",
      "Commandes illimitées avec paiement à la livraison (COD)",
      "Formulaire de commande express & contact WhatsApp",
      "Suivi des commandes et gestion des clients",
      "14 jours d'essai gratuit complet sans carte bancaire",
    ],
  },
  {
    name: "Starter",
    price: "7 900",
    note: "Pour vendre sérieusement en marque blanche avec l'IA.",
    recommended: false,
    icon: Rocket,
    cta: "Choisir Starter",
    items: [
      ...
      "Jusqu'à 200 produits dans votre catalogue",
      ...
      "Analyses et statistiques des visites en temps réel",
      "1 membre d'équipe (collaborateur ou livreur)",
      "Support e-mail réactif sous 24 h",
    ],
  },
  {
    name: "Pro",
    price: "14 900",
    note: "Débloquez tout le potentiel de votre marque et échellez.",
    recommended: true,
    icon: Warehouse,
    cta: "Choisir Pro",
    items: [
      "Sans badge DUKAIO (Marque blanche totale)",
      "40 créations complètes par IA par mois",
      "Produits illimités & jusqu'à 5 boutiques",
      "Nom de domaine personnalisé (votre-marque.com)",
      "Accès illimité & prioritaire à l'espace Découverte et Tendances",
      "Recherche et filtres concurrentiels avancés",
      "Relance automatique des paniers abandonnés par e-mail",
      "Jusqu'à 5 membres d'équipe (closers, gestionnaires, livreurs)",
      "Robot de commande WhatsApp (accès prioritaire)",
          ...
          >
            {index > 0 && (
              <>
                <i
                  aria-hidden
                  className={`absolute -left-[7px] -top-[7px] ${plan.recommended ? "blueprint-cross-light" : "blueprint-cross"}`}
                />
                <i
                  aria-hidden
                  className={`absolute -bottom-[7px] -left-[7px] ${plan.recommended ? "blueprint-cross-light" : "blueprint-cross"}`}
                />
              </>
            )}
      ...
      <div className="section-shell grid gap-10 py-16 lg:grid-cols-2 lg:items-center">
        <div>
          ...
          <p className="mt-5 max-w-xl text-background/70">
            Catalogue, commandes, clients et performances ne sont plus dispersés. Chaque écran
            répond à une action concrète.
          </p>
        </div>
      ...
      <div className="section-shell grid gap-10 py-16 md:grid-cols-[1fr_1.2fr] md:items-center">
        ...
        <div>
          ...
          <p className="mt-5 text-lg font-medium text-muted-foreground">
            Préparez une fiche complète, améliorez sa présentation et trouvez des angles marketing
            utiles sans quitter votre flux de travail.
          </p>
